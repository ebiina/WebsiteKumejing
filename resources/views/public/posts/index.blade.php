    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        ...
        <article class="bg-white rounded-2xl shadow-sm overflow-hidden group hover:shadow-xl transition duration-300 border flex flex-col">
            <div class="relative h-56 overflow-hidden">
                @if($post->image)
                <img src="{{ $post->image_url }}" alt="{{ $post->title }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                @else
                <div class="w-full h-full bg-village-light flex items-center justify-center p-12">
                <img src="/assets/LOGO KAB WONOSOBO.png" alt="Kumejing" class="max-h-full opacity-20 grayscale">
                </div>
                @endif
